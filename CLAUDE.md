...
* Battle (`components/Duel.jsx`, schema in §2): live matches for 2–8 players, joined by a 4-character room code. The creator picks the number of questions (5–30 chips or 3–50 custom), categories and 3-choice mode, then starts the match once everyone is in the lobby. Each question has a 30-second server-timed clock and closes early when everyone has answered. Live scoreboard that reorders by score and shows each player's state, everyone's pick tagged on the answers during the reveal, final standings with places and times, a per-question review that replays each question in full — every answer choice as it appeared, the correct one marked, each player's pick tagged on the choice they took, anyone who ran out of time named, and the explanation underneath. It covers every question, not only the ones you missed (asked for on 2026-09-23: a lucky guess is still worth reading up on). The option markup reuses the live card's `.q-option` classes with `.review-options` to undo the hover state, since these are plain divs, and a win/loss/draw tally. Updates come over Supabase Realtime with a 3-second polling fallback, and the active match id is kept in sessionStorage so a refresh rejoins it. Quitting drops you from the standings; the rest play on.
...
**Request path.** The browser posts `{userId, question, context, history, stream: true}`. The function then, in order: looks for a cached answer, checks `chat_quota` (see §2) with the Supabase anon key, and walks the provider chain until one starts answering. The reply **streams** back as newline-delimited JSON (`{"t":"..."}` per piece, then `{"done":true,"remaining":n}`); `AskAI` appends each piece to a live bubble. Errors and cached answers come back as ordinary JSON instead, which is how the client tells the two apart (`content-type`).

**Streaming is opt-in, and that matters.** Only a request carrying `stream: true` gets newline-delimited JSON; anything else gets the whole answer as one JSON object. This exists because shipping streaming broke every tab that had been open beforehand — old JavaScript reads the reply with `JSON.parse`, and newline-delimited JSON isn't valid JSON, so every question from a cached page failed while curl against the same endpoint looked perfect. Don't change the wire format without keeping a path for clients that predate the change. `AskAI` also retries once, silently, when it gets a non-JSON error body, which is what a deploy looks like from the browser.
...
**Grounded in the chart, but only so far.** An early live answer called Lay on the Table a privileged motion (it is subsidiary), so `_lib/facts.js` now feeds `src/data/motions.js` into the system prompt as ground truth. That fix then caused a worse one: told to trust the chart, the model replied that incidental main motions don't exist. They do (RONR 12th ed. 6:2, 10:4). The prompt now says the chart is authoritative for what it lists and explicitly not a complete account of the book, with the main-motion subclasses written out. Both failures were confident and fluent — that is the shape this model's errors take, so check answers about classification against the chart.
